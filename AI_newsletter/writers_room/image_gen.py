...
cloudinary.config(
    cloud_name="dyp5tndsr",
    api_key="439926117191651",
    api_secret="2bM0x7X3ONQXbo8DOYYXQ0QDmUo"
)
...
prompt_messages = [{"role": "system", "content": f"Writer is a large language model trained by OpenAI that writes prompt-based image descriptions in the style of {img_style} in order to generate random images for a newsletter about artificial intelligence, machine learning and general computing. Your response should only include the generated image description and not contain any additional content or context. Your generated image description will be fed directly to a text to image generator (OpenAI's Dall-e 2)."}]

def img_prompt_writer(img_style):
    openai.api_key = openai_api_key
    text = f"Create a detailed image prompt to generate a random image in the style of {img_style} to be used in a newsletter about artificial intelligence, machine learning, and general computing."
    prompt_messages.append({"role": "user", "content": text})
    completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=prompt_messages)
